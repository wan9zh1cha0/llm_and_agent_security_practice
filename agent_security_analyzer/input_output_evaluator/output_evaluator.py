...
class LlamaGuard_scanner:
    ...
    Task: Check if there is unsafe content in 'Agent' messages according to our safety policy. 
    ...
    def _prepare_input_response(self, prompt: str, reponse: str) -> list[dict[str, str]]:
    ...
    def classify_response(self, prompt: str, response: str) -> scan_result:
        input_response = self._prepare_input_response(prompt, response)
        output = self._generate(input_response)
